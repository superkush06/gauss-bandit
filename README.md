...
**Bandit algorithms, checked against the regret bound each one claims.**

A multi-armed bandit library in Python: seven policies, three environments, and
a harness that runs them. The hard part is not the algorithms, it is the
yardstick — a regret curve going up tells you nothing about whether the number
at the end is any good. Lai and Robbins settled that in 1985: for a given
instance there is a constant $C$, computable from the environment, such that no
consistent policy can have regret below $C \ln T$. This library computes $C$,
implements a policy that provably attains it, and measures everything else
against it — in the figures below, in [`docs/validation.md`](docs/validation.md),
and in the test suite.

![regret per ln T against the Lai-Robbins floor](docs/optimality.png)

The y-axis is regret divided by $\ln T$. A policy with logarithmic regret
flattens out, a policy with linear regret keeps climbing, and the dashed line
is the floor. **KL-UCB** and **Thompson** flatten and stay under it. **UCB1**
goes straight through — 3x above the floor on the left, 11x on the right, where
the arm means sit near 0.01 and its Hoeffding bonus is at its loosest. Curves
below the line are not a contradiction: the bound is asymptotic, and both
optimal policies approach it from underneath.
...
105 tests, about 25 seconds. At runtime the library needs Python and NumPy and
nothing else. `dev` adds pytest and ruff; `plot` adds matplotlib, which only
`docs/figures.py` uses.
...
The runner seeds both factories. `env_factory(seed)` and
...
The same divergence drives KL-UCB's index, which is why the policy meets the
bound. Here is its confidence bonus next to the Hoeffding bonus UCB1 uses, at a
per-arm budget of 0.001 on an arm averaging 2%:
...
KL-UCB's bonus is 0.0069 wide, UCB1's is 0.0224 — 3.2x narrower. Every unit of
unearned optimism is a pull spent on the wrong arm.

## Where the policies actually land

`python3 examples/optimality.py` — five arms with means from 0.10
down to 0.01, ten replications, about seven seconds:
...
arrives — that is what asymptotic optimality looks like at finite T, and the
chart at the top carries it out to T = 200,000. UCB1 crosses 1.00 before
T = 1,000 and keeps going: still logarithmic, but with a constant eight times
larger than it needs to be. ε-greedy is not logarithmic at all — its ratio goes
0.95 to 2.61 between T = 5,000 and T = 20,000 and does not stop.
...
because 2,000 rounds is nowhere near asymptotic. Read the standard deviations
too: annealed ε-greedy beats UCB1 on the mean, 84.98 against 224.80, with seven
times the spread, 129.69 against 18.45. A mean-only table hides that.

## Does it meet the bounds it cites?

Citing a theorem in a docstring is free.
[`docs/validation.md`](docs/validation.md) measures every policy against the
guarantee it claims, next to the script that produced the number.

`python3 examples/validate.py --long` (about 4 minutes)
...
regret over fourteen rounds can be computed exactly: enumerate all $2^{14}$ =
16,384 reward paths and weight each by its probability. No sampling, no bound,
no citation to get wrong. It agrees with 200,000 simulated runs to about one
standard error (z = 1.03), which checks the environment, the index and the
regret accounting at the same time.

Two claims do not come out clean, and the doc says so rather than burying them.
KL-UCB and Thompson are both *proved* asymptotically optimal, and neither has
reached the floor by T = 200,000 — they sit at 0.88 and 0.53 of it. LinUCB's
measured regret exponent is 0.006, so far under the $\sqrt{T}$ rate that it
certifies nothing about the implementation.

`tests/test_invariants.py` draws hundreds of random instances from a seeded
generator and asserts the algebra rather than stored expected values: Pinsker's
inequality on every random pair, EXP3's distribution summing to one with its
γ/K floor intact after any history, the Gaussian Lai-Robbins constant scaling
exactly as σ², LinUCB's inverse staying symmetric positive definite and its
choices unchanged when the entire problem is rotated.
...
100,000-round runs do not overflow. Its default exploration rate is the
decaying schedule $\gamma_t = \sqrt{K \ln K / ((e-1)\,t)}$: the tuned $\gamma$
of Auer et al. (2002), Cor. 3.2 with the horizon replaced by the current round.
That is standard practice, but it inherits none of the corollary's guarantee —
that one is for a single fixed $\gamma$, and the paper's unknown-horizon
algorithm is Exp3.1 (§4), which doubles a guess at $G_{\max}$ instead. The
effect is measured rather than cited: any fixed $\gamma$ pays a $\gamma t$ tax
forever, and the schedule does not.
...
schedule's line bends. The dotted marker sits at round 9,000, where linear-space
weights used to overflow to `inf`; after that the sampler produced `NaN`
probabilities and the policy silently pulled one arm for the remaining 91,000
rounds. The suite now runs the full 100,000 rounds and asserts the weights stay
finite.
...
Five arms, six-dimensional contexts, $\theta_a$ drawn at random, 30
replications. Every arm has the same *marginal* mean, so a context-free policy
has nothing to learn: UCB1 accrues 11,091 regret over 4,000 rounds against
LinUCB's 49, and only 1.2 of LinUCB's arrives after round 500. Each arm's
inverse design matrix is maintained by Sherman-Morrison rank-1 updates instead
of being refactorised every round. The tests pin it to `np.linalg.inv` within
1e-10 and require the two implementations to choose identical arms for 1,000
rounds.
...
python3 docs/figures.py optimality   # ~8 min: 2 instances, 4 policies, 20 runs, 200k rounds
```

Needs the `plot` extra. Every figure prints the table behind it before saving,
so the numbers in this README and the curves above them come from the same run.
...
behind LinUCB. References at the end.
...
`python3 examples/execution_router.py` is one worked instance — a $250M parent
order split into 20,000 child slices across four venues whose passive-fill
rates the router has to learn while it trades:
...
the other 0.169 bp ($4,222) is allocation waste. The bound is what tells you
which part is worth fixing. (Both adaptive routers land slightly under the
asymptotic floor at this horizon — the finite-T caveat in
`docs/validation.md`, not a broken theorem.)

The example imports nothing but this library. The pieces it stands in for live
elsewhere:
...
- **mlrun** — for tracking forty comparisons like the one above instead of one.
...
- **Pure Python.** Redrawing the `optimality` figure — two instances, four
  policies, twenty seeds, 200,000 rounds — takes about eight minutes on one
  core. That is a deliberate trade, because the algorithms are meant to be
  read, but this is not a vectorised benchmark harness.
